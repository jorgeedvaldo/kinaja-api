...
@section('page-title', 'Candidaturas: Motoristas')
@section('title', 'Candidaturas Motoristas')
...
<div class="table-card" style="margin-bottom: 24px; padding: 16px;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
        <span class="table-title" style="margin: 0;">Candidaturas de Motoristas ({{ $applications->total() }})</span>
    </div>
    <form method="GET" action="{{ route('admin.driver_applications.index') }}" style="margin:0; background: #f8fafc; padding: 16px; border-radius: 8px; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 12px; align-items: end;">
        <div>
        ...
        <div style="display: flex; gap: 8px;">
            ...
            <a href="{{ route('admin.driver_applications.index') }}" class="btn btn-secondary">Limpar</a>
        </div>
    </form>
</div>
...
<div class="table-card">
    ...
    <table>
        <thead><tr><th>ID</th><th>Nome</th><th>Telefone</th><th>Mota Própria</th><th>Status</th><th>Data</th><th>Ações</th></tr></thead>
        <tbody>
            ...
            <tr class="{{ $app->status === 'pending' ? 'bg-warning-light' : '' }}" style="{{ $app->status === 'pending' ? 'background: #fffbeb;' : '' }}">
                ...
                <td>{{ $app->phone }}</td>
                <td>{{ $app->owns_motorcycle ? 'Sim' : 'Não' }}</td>
                <td>
                ...
                <td>
                    <a href="{{ route('admin.driver_applications.show', $app) }}" class="btn btn-sm btn-outline">Ver Detalhes</a>
                </td>
            </tr>
            @endforeach
        </tbody>
